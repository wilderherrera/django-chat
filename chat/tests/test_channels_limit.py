...
class MyTests(TestCase):
    ...
    @override_settings(SOCKET_THROTTLE={"window": 1, "limit": 1})
    async def test_my_consumer_with_limit(self):
        ...
        await communicator.send_to(text_data=json_data)
        response = await communicator.receive_from()
        assert "Rate limit exceeded. Please wait before sending more messages." in response
        # Close
